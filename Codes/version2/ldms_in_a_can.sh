...
test_stream_publish () {
  ldmsd_stream_publish -x sock -h localhost -p ${SAMP_PORT} -t string -s amd_gpu_sampler -a none -f sampler.conf
}
...
# Loading the json_stream_sampler plugin
#load name=json_stream_sampler
#config name=json_stream_sampler path=/local_data/amd_gpu_sampler_data container=gpu_sampler_data stream=amd_gpu_sampler buffer=0
